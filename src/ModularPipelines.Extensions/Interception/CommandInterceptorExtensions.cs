...
{
    public static IServiceCollection AddCommandInterceptor<T>(this IServiceCollection services) where T : class, ICommandInterceptor => services.AddScoped<ICommandInterceptor, T>();
    public static IServiceCollection AddCommandInterceptors<T>(this IServiceCollection services) where T : class
    {
        ...
        {
            static T factory(IServiceProvider provider) => provider.GetRequiredService<T>();
            services.AddScoped<T>();
            services.AddScoped(item, (Func<IServiceProvider, T>)factory);
            services.AddScoped(typeof(ICommandInterceptor), typeof(TypedInterceptor<>).MakeGenericType(item.GetGenericArguments()[0]));
        }
        return services;
    }

    public static IServiceCollection AddCommandInterceptor(this IServiceCollection services, Func<CommandLineToolOptions, CommandExecutionOptions?, CancellationToken, ValueTask<(CommandLineToolOptions Options, CommandExecutionOptions? ExecutionOptions)>> interceptFunc) => services.AddScoped<ICommandInterceptor>(_ => new InlineInterceptor(interceptFunc));
    ...
    where TOptions : CommandLineToolOptions => services.AddScoped<ICommandInterceptor>(_ => new InlineOptionsCommandInterceptor<TOptions>(interceptFunc));
