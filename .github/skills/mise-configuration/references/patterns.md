...
{% endif %}SRC_EXISTS = "true"# Directory checkVERSION     = "{{ read_file(path='VERSION') | trim }}"# File operationsARCH        = "{{ arch() }}"                               # x64, arm64# System infoBUILD_TIME  = "{{ exec(command='date +%Y-%m-%d') }}"# Execute shell commandNODE_VER    = "{{ get_env(name='NODE_VERSION', default='20') }}"# Get env var with fallback
